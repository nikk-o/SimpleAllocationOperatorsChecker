...
namespace {
  class DeleteOperatorChecker : public Checker<check::PostStmt<CXXNewExpr>,
...
DeleteOperatorChecker::DeleteOperatorChecker(){
  ...
  resourceLeak.reset(
    new BugType(this, "ResourceLeak", "BugCategory")),
  ...
  useAfterDelete.reset(
    new BugType(this, "UseAfterDelete", "BugCategory"));
}
...
void DeleteOperatorChecker::checkPreStmt(const CXXDeleteExpr* expr, CheckerContext& checkerContext) const{
  ...
  // It means we ran into delete statement on a memory location that isn't allocated 
  // (atleast not in the current scope/file?)
  if(asc == NULL){
    return;
  }
